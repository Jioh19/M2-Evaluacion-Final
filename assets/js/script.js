...
function newDigimon() {
	$.get("https://digimon-api.vercel.app/api/digimon", function (digimon, status) {
...
function digimonMore(name) {
	...
	$.get(dir, function (digimon, status) {
		console.log(`Estado: ${status}`);
		console.log(digimon);
        try {
				...
				.then((digimon) => {
					for (let i in digimon.descriptions) {
						if (digimon.descriptions[i].language == "en_us") {
							$("#description").text(digimon.descriptions[i].description);
						}
					}
				});
            } catch (err) {}
